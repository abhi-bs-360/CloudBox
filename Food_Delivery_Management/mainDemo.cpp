/* This below c.Header file is in the same directory as the current file.
    It contains all used libraries, structures, unions and Globals.. */
#include "library.c"
...
/* Function which adds final items to cart during payment */
void add_to_Cart(char *foodName, int foodCost, int foodCount, int abc) {

    strcpy(user[User.Id].final_order[abc], foodName);
    
    user[User.Id].cost_arr[abc] = foodCost;
    
    user[User.Id].count_arr[abc] = foodCount;
}
...
void display_details() {
    ...
    printf("\n*******************************");
    printf("\nTotal amount(inclusive of GST and discount) : %0.2f\n\n", user[R].damount);
...
//here var h = 0 (if customer) , else 1 (if hotel staff)

    int person = h;
    int serialno, count = 0, orderno = 0;

    int customer = 0, staff = 1;
    float amount = 0;
...
// Ordering portal to choose food required on basis of serial number..
    order: {
        ...
        if(user[User.Id].offerCheck == 1) {
            
            printf("\nSorry You cannot order further since you have applied offer Coupon!!\n");
            
            printf("Proceed to payment....\n");
            goto cdef;
        }
        
        int vv, serialN, check = 0;
        ...
        for(vv = 1; vv <= items; ++vv) {

            if(serialN == x[vv].foodno) {
                serialno = vv;
                check = 1;
                break;
            }
...
// Offer coupons
    offer: {
        ...
        if(user[User.Id].offerCheck == 1) {
            ...
            printf("\nOffer Coupon already applied! Proceed to payment\n");
            goto cdef;
        ...
        printf("\nIF YOU HAVE ANY OFFER COUPON ENTER 1 , ELSE 2 TO CONTINUE : "); {
            ...
            if(a == 1) {
                ...
                for(int i = 1; i <= offcount; i++) {
                    ...
                    if(b == off[i].code) {
                        ...
                        user[User.Id].damount = amount - amount * (((float)off[i].discount)/100);
                        
                        user[User.Id].offerCheck = 1;
                        
                        break;
                    }
                }

                if(user[User.Id].offerCheck == 0) {
                    printf("\nINVALID Offer CODE!!!\n");
                    goto offer;
                }
            }
            ...
            printf("\nSorry!! The minimum expenditure must be Rs.250 to use coupons :( \n");
        ...
        if (user[User.Id].damount > 0) {
            ...
            for(int i = 1; i <= items; i++) {
                ...
                if(x[i].mark == 1) {
                    ...
                    // cart function : 15 < nLine < 30
                    add_to_Cart(x[i].food, x[i].cost, x[i].count, abc);
                    abc += 1;         
                }
            }
...
// To delete food items if required..
    Delete: {
        ...
        if(amount > 0) {
            ...
            for(cc = 1; cc <= items; ++cc) {

                if((x[cc].mark == 1) && (a == x[cc].foodno)) {
                    a = cc;
                    ss = 1;
                    break;
                }
...
void set_password() {
    ...
    // validator function : 100 < nLine < 160
...
void create_account(int F) {
	...
	// F argument can be 0 or non-0 depending on signUP or Editing
...
void user_login() {
    ...
    // serach algo function : 30 < nline < 50
    int newID = User.Id;
	...
	if(strcmp(key, user[newID].email) == 0) {
		...
		if(strcmp(key1, user[newID].password) == 0) {
            ...
            // argument 0 for customers to login..
            order_food(0);
		}
...
void official_login() {
	...
	if(strcmp(id, emp.official_id) == 0) {
		...
		if(strcmp(code, emp.official_password) == 0) {
            ...
            // Argument 1 says officials has logged in..
            order_food(1);
...
int main() {
	...
	if(random == 1) {
        ...
        // delay function : Start of the code
        delay(800);
